...
import { Menu, X, LayoutDashboard, ShoppingBag, Users, Warehouse, Truck, LifeBuoy, Activity, LogOut, Search } from 'lucide-react'
...
export default function AdminLayout({
    ...
    const opsNav = [
        { label: 'Stock Inventory', href: '/admin/inventory', icon: Warehouse },
        { label: 'Global Shipments', href: '/admin/shipments', icon: Truck },
    ...
    const renderNav = (closeMobile = false) => (
        <nav className="space-y-1 px-3">
            ...
            <div className="px-3 pt-6 mb-2 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Logistics & Ops</div>
            {opsNav.map((item) => {
                const IconComp = item.icon
                const isActive = pathname === item.href
                return (
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen bg-zinc-950 font-sans text-white overflow-hidden">
            ...
            <div className="md:hidden bg-zinc-900 border-b border-zinc-800 px-4 py-3 flex items-center justify-between z-40 shrink-0">
                <div>
                    <div className="font-bold text-lg text-emerald-400">Admin Portal</div>
                </div>
                <div className="flex items-center gap-2">
                    <button
                        onClick={() => openCommandPalette()}
                        className="p-2 rounded-xl bg-zinc-800 text-emerald-400 hover:text-white"
                        aria-label="Search"
                    >
            ...
            {mobileOpen && (
                <div className="md:hidden fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex flex-col">
                    <div className="p-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-900">
                        <div className="font-bold text-lg text-emerald-400">Admin Operations</div>
            ...
            <aside className="hidden md:flex w-64 bg-zinc-900 border-r border-zinc-800 flex-col shrink-0">
                <div className="p-6 border-b border-zinc-800">
                    ...
                    <div className="text-xs text-zinc-500 font-mono truncate mb-4">Operations & Dispatch</div>

                    <button
                        onClick={() => openCommandPalette()}
                        className="w-full px-3 py-2 rounded-xl bg-zinc-950 border border-zinc-800 text-zinc-400 hover:text-white text-xs font-mono flex items-center justify-between transition-all hover:border-emerald-500/50"
                    >
