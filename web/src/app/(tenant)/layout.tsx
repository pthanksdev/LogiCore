...
export default function TenantLayout({
    ...
    const mainNav = [
        ...
        { label: 'Module Catalog', href: '/catalog', icon: Package },
        { label: 'Orders & Tracking', href: '/orders', icon: ShoppingCart },
        { label: 'Notifications', href: '/notifications', icon: Bell },
        { label: 'Profile Settings', href: '/settings', icon: Settings },
    ]

    const serviceNav = [
        { label: 'Cargo Shipments', href: '/dashboard/shipments', icon: Truck },
        { label: 'Invoices & Billing', href: '/dashboard/billing', icon: CreditCard },
        { label: 'Team Directory', href: '/dashboard/team', icon: Users },
        { label: 'API & Webhooks', href: '/dashboard/integrations', icon: Key },
    ]
    ...
    const renderNav = (closeMobile = false) => (
        <nav className="space-y-1 px-3">
            ...
            <div className="px-3 pt-6 mb-2 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Services & Tools</div>
            {serviceNav.map((item) => {
                const IconComp = item.icon
                const isActive = pathname === item.href
                return (
